...
export async function POST(request: NextRequest) {
  const { storeUrl, accessToken, productsPerPage, currentPage } =
    await request.json();
  const headers = {
    "Content-Type": "application/json",
    Authorization: `Bearer ${accessToken}`,
  ...
  try {
    const response = await axios.get(
      `${storeUrl}/rest/default/V1/products/?searchCriteria[PageSize]=${productsPerPage}&searchCriteria[currentPage]=${currentPage}&fields=items[id,sku,name,price],total_count`,
      {
        headers,
      }
    );
